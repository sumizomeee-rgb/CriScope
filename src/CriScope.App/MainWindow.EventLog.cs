...
public sealed partial class MainWindow
{
    ...
    private bool _logFollowing = true, _updatingLog;
    ...
    private Control BuildEventLog()
    {
        ...
        var bar = new Grid { ColumnDefinitions = new ColumnDefinitions("*,100,Auto,Auto"), Margin = new Thickness(12,5), ColumnSpacing=8 };
        ...
        var headings=new Grid{ColumnDefinitions=new ColumnDefinitions("100,90,*,*"),ColumnSpacing=8,Margin=new Thickness(12,0)};
    ...
    private void UpdateEventLog()
    {
        ...
        var matching=source.Where(e=>EventLogPresentation.Includes(e,_logKind)&&EventLogPresentation.Matches(e,_logQuery,Cue(e))).ToArray();
        var results=matching.TakeLast(_logLimit).Reverse().ToArray();
        if(_logStatus!=null)_logStatus.Text=$"{(_logFollowing?"实时更新 · 最新在上":"暂停刷新 · 采集继续")} · {results.Length} / {matching.Length} 条匹配 · 双击定位 · 搜索当前缓存，打开日志可查完整录制";
        double timeOrigin=_session?.TimeOrigin??_timeline.TimeOrigin;
        var signature=(_p.Light?"L":"D")+timeOrigin+_logQuery+_logKind+string.Join(',',results.Select(e=>e.session+":"+e.seq));
        ...
        try
        {
            ...
            {
                var line=new Grid{ColumnDefinitions=new ColumnDefinitions("100,90,*,*"),ColumnSpacing=8,Margin=new Thickness(0,2)};
                ...
                var name=Label(Cue(e).Length>0?Cue(e):e.name,12);name.TextTrimming=TextTrimming.CharacterEllipsis;Grid.SetColumn(name,2);line.Children.Add(name);
                string detail=EventLogPresentation.Detail(e);
                var owner=owners.GetValueOrDefault(e.entity=="cue"?e.objectId:e.parentId);
                if(owner!=null && (EventSemantics.IsPlaybackEnd(e) || e.kind=="stop") && owner.DurationAt(e.time) is {} duration)detail+=$" · {duration:0.000} 秒";
