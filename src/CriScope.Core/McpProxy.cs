        ...
        while ((line = await Console.In.ReadLineAsync()) != null)
        {
            ...
            try
            {
                ...
                if (method == "initialize") result = new { protocolVersion = "2024-11-05", capabilities = new { tools = new { } }, serverInfo = new { name = "criscope", version = "0.2.0" } };
                    ...
                    Tool("control_ui", "操作与人工共享的界面状态。action: workspace/live/filter/select/session/range/theme/diagnostics/fit/control-kind/spatial-layer/navigate/back/log-search/log-follow；range的value为来源时间起点:终点，select为事件seq；navigate为space或playback，log-search检索事件日志；fit为全览，live返回最近窗口", new { action=new { type="string" }, value=new { type="string" } }, new[]{"action"}),
